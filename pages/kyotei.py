...
jst\_now = datetime.datetime.utcnow() + datetime.timedelta(hours=9)
today\_str = jst\_now\.strftime("%Y%m%d")
time\_now = jst\_now\.strftime("%Y-%m-%d %H:%M:%S")
...
st.set\_page\_config(page\_title="競艇AI予想", layout="wide")
st.title("🚤 競艇AI予想 - 出走表＋予想結果")
st.write(f"📅 本日：{today\_str}")
st.write(f"🕒 現在時刻（日本時間）：{time\_now}")
...
st.write("### 🤖 AI予想")
prediction = predict\_ai(race\["選手"])
